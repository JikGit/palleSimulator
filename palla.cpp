...
class Palla
{
    ...
    public:
        ...
        void hitting(Palla& p) {
            const float m1 = this->mass, m2 = p.getMass();
            const float pos1x = this->pos.x, pos1y = this->pos.y, pos2x = p.getPos().x, pos2y = p.getPos().y;
            const float vel1x = this->vel.x, vel1y = this->vel.y, vel2x = p.getVel().x, vel2y = p.getVel().y;
